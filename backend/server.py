...
async def create_chat_room(room: dict):
    ...
    new_room = ChatRoomModel(
        ...
        maxMembers=int(room.get('maxMembers', 50)),
...
@api_router.patch("/social/chat-room/{room_id}")
async def update_chat_room(room_id: str, data: dict):
    allowed = {k: v for k, v in data.items() if k in {"name", "description", "type", "maxMembers"}}
    if not allowed:
        raise HTTPException(status_code=400, detail="Nothing to update")
    await db.chat_rooms.update_one({"id": room_id}, {"$set": allowed})
    room = await db.chat_rooms.find_one({"id": room_id})
    if not room:
        raise HTTPException(status_code=404, detail="Room not found")
    room["lastActivity"] = datetime.utcnow()
    await db.chat_rooms.update_one({"id": room_id}, {"$set": {"lastActivity": room["lastActivity"]}})
...
async def post_chat_room_message(data: dict):
    ...
    msg = {
        ...
        "meta": data.get('meta')  # optional extra data (e.g., shared post id)
    }
